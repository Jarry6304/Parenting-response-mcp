...
components: resonance-c-light.md（v3,已產）, references/cores/*.md（已產）, references/record-schema.md（已產）, references/pingpong.md（已產）, references/tw-parenting-antipatterns.md（已產）
---
...
| 合成v3 | resonance-c-light v3 連動:關鍵決策「同模+家族折算」→「不分族、不折算」;SYN 改隔離並列(洗牌)+溯源;`resonance_trace`→`synthesis_trace`;rounds 加 `core_outputs`(trace 瘦身後的 A3 來源) | 連動修改 |
...
- 架構/核心/L0 欄位/路線圖/風險 → `parenting-response-spec-v2.md`
- 合成契約(隔離並列/溯源)/ synthesis_trace schema → `resonance-c-light.md`(v3)
...
| **不分族、不折算(resonance v3)** | 家族折算 / 不數票 | 計票機制移除後家族標籤無消費端;同模相關性的安全面由 G0+後檢承擔,風味偏置由 L1 以 utterance_sources 實證再議 |
| 合成取捨 = LLM 生成(C-輕 v3) | code 評分 / LLM 生成 | 質性取捨需語意一致性,交 LLM;硬規則(溯源驗證/禁詞/約束)靠 code 閘 |
...
| 合成 | C-輕(見 resonance-c-light v3):隔離並列(洗牌)→ 生成 + 溯源 | 1 次 LLM 呼叫 + code 排版/洗牌/溯源驗證 |
...
# AnalyzeResult = { session_id, card: Card | None, synthesis_trace: Trace | None,
...
# RoundResult = { card: Card | None, synthesis_trace: Trace | None, converged: bool,
#                 redflag: {hit, reason, referral} | None }
...
家族折算已廢(resonance v3:不分族、不折算、不加權)。隔離 = 每核心一次獨立 API 呼叫,無共享 context。
...
flowchart TD
    ...
    FAN --> COL[code 收集+隔離並列排版（洗牌）]
    COL --> SYN[C-輕 合成 LLM<br/>讀並列全文+約束 → 生成+溯源標記]
    SYN --> CHK{後檢:code pattern + guardian}
    CHK -->|違約束,retry ≤ N| SYN
    CHK -->|超限 或 約束核心可用<K| DEG[降級安全卡<br/>degraded=true]
    CHK -->|過| PERS
    DEG --> PERS
    PERS --> RESP[回 card + synthesis_trace]
```
...
CREATE TABLE rounds (
    ...
    card            JSONB NOT NULL,
    core_outputs    JSONB NOT NULL,                -- 各核心原始輸出,key=核心 id(隔離審計 + A3 聚合來源;合成v3 trace 瘦身連動)
    synthesis_trace JSONB NOT NULL,                -- 溯源審計,schema_version=3,見 resonance-c-light.md
...
> 約束核心的 `analysis` 進合成當脈絡、`constraints` 雙端供給:合成(事前遵守,降 retry)+ 後檢(硬保證,逐條驗)(resonance v3)。
...
├── record-schema.md           # L0 欄位 + 詞表 + A3 聚合回填規則—已產
├── pingpong.md                # S3 反應路由 + converged 判準—已產
└── tw-parenting-antipatterns.md  # 反模式語料 F1–F8 / P01–P50,雙消費端—已產
```
...
- [ ] 每輪 card + core_outputs + synthesis_trace 落 `rounds`;finalize 落 `records`

## 漸進載入指引

| 任務 | 查閱 |
|---|---|
| 合成契約(隔離並列/溯源)/ synthesis_trace schema | `resonance-c-light.md`(v3,已產) |
| 各核心 prompt(産招話術 / 約束規則) | `references/cores/<name>.md`(已產) |
| L0 欄位語意 + 受控詞表 + A3 回填規則 | `references/record-schema.md`(已產) |
| S3 反應 → 點火路由 + converged 判準 | `references/pingpong.md`(已產) |
| G0 詞源 / pattern 詞表 / confounders 詞源 | `references/tw-parenting-antipatterns.md`(已產) |
| 架構 / 路線圖 / 風險 | `parenting-response-spec-v2.md` |
