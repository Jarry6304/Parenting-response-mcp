...
next_round(session_id: str, child_reaction: str) -> RoundResult   # S3 乒乓
# child_reaction ∈ {鬆動配合,否認堅持,情緒爆發,退縮害怕,反問試探,轉移打岔}
# FSM 守衛 → G0 複檢（對 child_reaction 文本）→ 點火相關核心（單波）→ 重合成 → 後檢 → append round
...
| next_round | session 存在 ∧ `status=open` ∧ round 0 存在;`child_reaction` ∈ 六類;對 child_reaction 文本重跑 G0 | `E_INVALID_STATE` / `E_INVALID_REACTION` |
...
`next_round`:同管線,差三處——守衛多查 `status=open`、G0 改對 `child_reaction` 文本複檢(命中 → 自動落 record `escalated_to_redflag` + 轉終態)、fan-out 換 pingpong 路由的點火子集(見 `references/pingpong.md`),append round。
...
G0 紅旗（情境,code）         : analyze 命中 → 短路;next_round 對 child_reaction 複檢,命中 → 自動收案轉終態
