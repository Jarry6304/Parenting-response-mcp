...
version: 0.3
status: DRAFT（schema_version 3 隨 spec v3.2 落地;v2/v1 節保留供讀舊列）
date: 2026-06-12
implements: parenting-response-mcp-spec-v3.2.md(records v3 + side-tables);歷史:v3.0(records v2)、v2.2(records v1 / A3)
...
## schema_version 3(v3.2,現行)

v3.2 G0 訊號化 + retro:在 v2 基礎上新增——
...
| sessions.`redflag_active`(新增) | bool 單調只升:①②③④⑤ 任一入口短路命中 → true;不再停案(`redflag_stopped` 自 v3.2 不再產生) |
...
| 欄位 | 值域 | 語意備註 |
|---|---|---|
| sessions.mode | `live` \| `rehearsal` \| `retro` | live = 現場;rehearsal = 預演;retro = 事後覆盤(v3.2 B 件:② 必填 parent_action、③×1 回覆盤鏡頭、converged=NULL) |
| sessions.caregiver | `爸` \| `媽` | v3.2 K 件;由已驗 sub 映射,不收輸入 |
| sessions.redflag_vector | `child` \| `parent` \| `third` \| NULL | v3.2 G 件;首見不覆寫 |
| sessions.status | `open` \| `finalized` \| `expired` \|(legacy:`redflag_stopped`) | `expired` = TTL 棄案(自最後活動——含 updated_at——逾 `SESSION_TTL_DAYS` 由 ① 懶清掃轉入;無 record,severity 留存);`redflag_stopped` 僅 v3.0 歷史列,v3.2 起不再產生,查詢視同 closed |
...
|---|---|---|
| outcome=escalated_to_redflag(v3.2 起僅 host 自報;**優先於 mode**) | `stopped`(legacy 映射) | v3.2 promotion 排除主錨 = `record.redflag`,本列為雙保險;v3.0 歷史列另有 ③ 自動收案來源 |
...
> append-only 證據鏈:G0 命中、拒收與報告稽核的「為什麼」落庫於此——`severity=高` 不再是無緣由孤值;「曾接觸紅旗之案」= `events.kind=g0_shortcircuit` 之 session 集合(v3.2 主錨)∪ `records.redflag=true`。本表非 records 欄位,**不動 `schema_version`**;payload 契約變更在本節追蹤。報告級/auth 事件 `session_id=NULL`,以 payload `ref_key`/`sub` 錨定;authkit 模式所有 payload 自動附 `sub`(誰觸發)。

| kind | 觸發 | payload |
|---|---|---|
| `g0_shortcircuit` | ①②③④⑤ 短路級命中(③ 另含 `round_no`) | `source`(①\|②\|③\|④\|⑤)、`field`、`phrase`、`excerpt`、`vector`(child\|parent\|third,v3.2)、`referral_delivered=true` |
...
- `schema_version` 現 = **3**(v3.2,migrations/0004)。**任何欄位語意或值域變更必須 bump**,並同步 Alembic 遷移;L1–L4 讀取端依版本分流解讀。side-tables(events/raw_transcripts/reports)為 append-only 契約,不動本版號,變更在各節追蹤。
