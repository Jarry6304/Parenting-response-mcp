...
> L0 紀錄是 L1–L4 的唯一資料源:欄位語意一旦寫入就是長期契約。本檔鎖定**受控詞表的值域**與**records 欄位的落庫規則**;讀取端依 `schema_version` 分流(v2 = 現行 thin server;v1 = fat server 歷史列,規則保留於後)。

## schema_version 2(v3.0,現行)
...
## events(稽核事件,defect-fixes #7/#8;migrations/0003)

> append-only 證據鏈:G0 命中與 ④ 拒收的「為什麼」落庫於此——`severity=高` 不再是無緣由孤值;「曾接觸紅旗之案」= `events.kind=g0_shortcircuit` ∪ `records.outcome=escalated_to_redflag`。本表非 records 欄位,**不動 `schema_version`**;payload 契約變更在本節追蹤。

| kind | 觸發 | payload |
|---|---|---|
| `g0_shortcircuit` | ①③④ 短路級命中(③ 另含 `round_no`) | `source`(①\|③\|④)、`field`(命中欄位)、`phrase`、`excerpt`(前後文節錄)、`referral_delivered=true` |
| `g0_warning` | ①③④ 警訊級命中 | `source`、`hits: [{field, phrase}, …]`(③ 另含 `round_no`) |
| `finalize_rejected` | ④ pattern 拒收(不落 record) | `violations`(命中詞列)、`outcome`(嘗試值)、`redflag_hit`(bool) |

## record_id 與 schema_version

- `record_id` = `YYYYMMDD-NN`(server 產,當日序號從 01 起;超過 99 自然增寬)。「當日」= **臺北日**(UTC+8 固定偏移,台灣無夏令時),與部署主機時區無關(defect-fixes #10)——UTC 容器或多機共庫不再出現跨午夜前綴錯日。
- `schema_version` 現 = **2**(v3.0,migrations/0002)。**任何欄位語意或值域變更必須 bump**,並同步 Alembic 遷移;L1–L4 讀取端依版本分流解讀。
