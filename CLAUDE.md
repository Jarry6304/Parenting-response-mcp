...
台灣家庭育兒回應 MCP server(v3.2 thin:**零 LLM 呼叫、零 API key**)。文件與訊息一律 zh-TW。
...
1. **`parenting-response-mcp-spec-v3.2.md` 是總規格(LOCKED)**;行為與規格衝突 = bug。原文待入庫(由規格持有者提交);v3.0 與更早(含 v2.2)僅供歷史參照。
...
- **G0 = 訊號,不是閘(v3.2 核心)**:①②③④⑤ 全入口複檢,短路命中**永不停案**——旗標 `redflag_active`/`severity=高`/`redflag_vector`(首見)單調寫入,FSM 照常推進;`redflag_stopped` 終態已退役,**任何新路徑不得產生**。強制力只在輸出匣:③ redflag_active → 安全約束集換軌(無一般管教 TAG、converged 恆 false)、④ 須 `referral_ack=true` 否則 `E_MISSING_AXIS`(訊號先落再擋)、`record.redflag=true` 永久排除 promotion(`E_INVALID_LINK`)。
...
- 部署/備份/還原/金鑰輪替操作 → `docs/deploy-runbook-v3.2.md`(雲端設定不進 code,只認 env)。
